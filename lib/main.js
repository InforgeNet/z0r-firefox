...
function handleClick(state) {
  ...
  var url = dcm.getElementById("urlbar").value;
    // match-pattern doesn't work properly in Firefox <= 31.* ??? (Need more tests...)
  if (xul_app.version >= 32 && !pattern.test(url)) {
    notifications.notify({
      text:"Invalid URL. Try Reloading Page.",
    });
    return;
  }
...
function handleResponse(response) {
  ...
  clipboard.set(short);
  notifications.notify({
    text:"URL shrinked: "+ short + " (Copied to Clipboard)",
  });
}
